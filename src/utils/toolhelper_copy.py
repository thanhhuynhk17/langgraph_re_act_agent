...
def run_normalization_data(sequences, path_stopwords: str):
    sequences = [remove_stopwords_vi(sequence, path_documents_vi=path_stopwords) for sequence in sequences]
...
def run_hybrid_search(model: SentenceTransformer, prompt: str, doc_embeddings, k: int):
    ...
    """
    path = f"src/hoanghamobile.csv"
    result_list = run_load_data_to_embedding(path)
    result_list = run_normalization_data(result_list, 'stopw')
