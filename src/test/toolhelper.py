...
def search_type_category_and_people(database: pd.DataFrame, value_type_of_food: str, value_option: str, num_people: int) -> pd.DataFrame:
    """
    ...
    Args:
        ...
        num_people (int): Number of people filter. 
            Example: 1, 2, 3 (matches values like "1 người", "2-3 người").
    ...
    search_cols = database.columns.drop(["type_of_food", "number_of_people_eating"])
    ...
    # 3️⃣ filter people
    mask_people = database["number_of_people_eating"].str.contains(str(num_people), na=False)

    # 4️⃣ combine
    result = database[mask_type & mask_any & mask_people].reset_index(drop=True)
    return result
